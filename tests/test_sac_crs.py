...
from rejax import SACCVaRRejectionSampling
...
class TestEnvironmentsSACCVaRRejectionSampling(unittest.TestCase):
    args = {
        "num_envs": 1,
    ...
    def train_fn(self, sac_crs):
        return SACCVaRRejectionSampling.train(sac_crs, rng=jax.random.PRNGKey(0))

    def test_env1(self):
        for discrete, env in enumerate([TestEnv1Continuous(), TestEnv1Discrete()]):
            with self.subTest(discrete=bool(discrete)):
                sac_crs = SACCVaRRejectionSampling.create(env=env, **self.args)
                ts, _ = self.train_fn(sac_crs)
                act = sac_crs.make_act(ts)
                ...
                if discrete:
                    q_fn = jax.vmap(
                        lambda *args: sac_crs.critic.apply(*args, method="take"),
                        in_axes=(0, None, None),
                    )
                else:
                    actions = jax.numpy.expand_dims(actions, 1)
                    q_fn = jax.vmap(sac_crs.critic.apply, in_axes=(0, None, None))
        ...
        for discrete, env in enumerate([TestEnv3Continuous(), TestEnv3Discrete()]):
            with self.subTest(discrete=bool(discrete)):
                ...
                for obs in jax.numpy.array([[-1], [1]]):
                    r = 1 * sac_crs.gamma if obs == -1 else 1
    ...
    def test_env5(self):
        for discrete, env in enumerate([TestEnv5Continuous(), TestEnv5Discrete()]):
            with self.subTest(discrete=bool(discrete)):
                ...
                if not discrete:
                    value = sac_crs.vmap_critic(ts.critic_ts.params, obs, obs)
