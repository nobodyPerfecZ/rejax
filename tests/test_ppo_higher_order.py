...
from rejax.algos.ppo_higher_order import PPOKurt, PPOSkew
...
class TestEnvironmentsPPOKurt(unittest.TestCase):
    args = {
        "kurt_coef": 1e-4,
        "num_envs": 64,
        "num_steps": 16,
        ...
        for discrete, env in enumerate([TestEnv5Continuous(), TestEnv5Discrete()]):
            with self.subTest(discrete=bool(discrete)):
                ...
                for o, a in zip(obs, actions):
                    if discrete:
                        self.assertEqual(a > 0.5, o.item() > 0)
                    else:
                        self.assertAlmostEqual(a, o.item(), delta=0.2)


class TestEnvironmentsPPOSkew(unittest.TestCase):
    args = {
        "skew_coef": 1e-3,
        "num_envs": 64,
        "num_steps": 16,
    ...
    def train_fn(self, ppo):
        return PPOSkew.train(ppo, rng=jax.random.PRNGKey(0))

    def test_env1(self):
        for discrete, env in enumerate([TestEnv1Continuous(), TestEnv1Discrete()]):
            with self.subTest(discrete=bool(discrete)):
                ppo = PPOSkew.create(env=env, **self.args)
                ts, _ = self.train_fn(ppo)
                value = ppo.critic.apply(ts.critic_ts.params, jax.numpy.array([0]))
                value, _ = jnp.split(value, 2, axis=-1)
                self.assertAlmostEqual(value, 1.0, delta=0.1)
