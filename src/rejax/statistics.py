...
def skewness(
    ...
    # Compute skewness as the third central moment divided by the square of the variance
...
def explained_variance(y_true: chex.Array, y_pred: chex.Array) -> chex.Array:
    """
    Compute the explained variance (EV).
    ...
    Returns:
        chex.Array:
            The explained variance (EV).
    """
    var_y = jnp.var(y_true)
    # Avoid division by zero if the variance of targets is zero
    # (e.g., at the very start of training or in a trivial environment)
    return jnp.where(var_y == 0, 0.0, 1 - jnp.var(y_true - y_pred) / var_y)
